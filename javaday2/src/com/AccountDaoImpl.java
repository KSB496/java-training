...
public class AccountDaoImpl implements AccountDao {
	...
	public void deposit(int accNo, float amt) throws CustomException{
		...
		{	
			...
			{
						totalAmount[i][1] +=amt;
						//System.out.println("totalamount : " +totalAmount[i][1]);
	...
	public float withdraw(int accNo, float amt) throws CustomException{
		if(accNo!=totalAmount[0][0]) {
			throw new AccountCreationException("Account not found");
		}
		if(amt <=0)
	...
	public boolean accountCheck(int accNo) throws CustomException {
		for(int i=0; i< totalAmount.length; i++)
		{
			if(totalAmount[i][0]==0 && (accNo!=0))
			{
				totalAmount[i][0]=accNo;
				return true;
			}	
